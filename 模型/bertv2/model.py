...
class Bertv2(nn.Module):
    def __init__(self,checkpoint='bert-base-cased',pretrained_freeze = False,dropout_rate = 0.3,hidden_size = 768,num_classes = 6,uniform_init=0.1,pooler_depth=4):
        ...
        self.num_classes = num_classes
        self.uniform_init= uniform_init
        self.pooler_depth = pooler_depth
        ...
        #模型head部分
        self.fc = nn.Sequential(
            nn.Linear(hidden_size*pooler_depth, hidden_size),
            nn.Dropout(p=dropout_rate),
            nn.Linear(hidden_size, num_classes)
            )
        #head部分的初始化
    ...
    def forward(self,input_ids,attention_mask,token_type_id):
        output = self.bert(input_ids,attention_mask,token_type_id ,return_dict  = True,output_hidden_states=True)
        hidden_states = torch.cat(tuple([output['hidden_states'][-i] for i in range(1,self.pooler_depth+1)]),dim=-1)#(batch,seq_len,hidden_dim*4)
        hidden_states = hidden_states[:,0,:]
        logits = self.fc(hidden_states)#logits的shape为(batch,num_classes)
        return logits
    ...
    def load(model_path):#模型的加载
        ...
        params = torch.load(model_path, map_location=lambda storage, loc: storage)
        args = params['args']
        model = Bertv2(**args)# **表示传入的是字典
        model.load_state_dict(params['state_dict'])
        return model

    def save(self, path):
        #在save中增加输出时间方便进行版本管理
        print('[%s] save model parameters to [%s]' % (time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))
        ...
        params = {
            'args': dict(checkpoint=self.checkpoint,
                         ...
                         num_classes = self.num_classes,
                         uniform_init=self.uniform_init,
                         pooler_depth = self.pooler_depth
                         ),
            'state_dict': self.state_dict()
        }
