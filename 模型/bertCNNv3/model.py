...
class BertCNN(nn.Module):
    ...
    def forward(self,input_ids,attention_mask,token_type_id):
        ...
        #text_cnn
        embeddings = output['hidden_states'][0]#只要embedding层
        embeddings = embeddings[:,1:-1,:]#batch,seq_len-2,hidden_state         
        embeddings = embeddings.permute(0,2,1)#batch,hidden_state,seq_len-2 
        feature_vector = torch.cat((self.conv_head_2(embeddings).squeeze(2),
                                    self.conv_head_3(embeddings).squeeze(2),
                                    self.conv_head_4(embeddings).squeeze(2)),1)#feature_vector的shape为batch,3*num_kernels
        feature_vector = self.conv_fc(feature_vector)
        
        #最终
        feature_vector = torch.cat((logits,feature_vector),1)
        logits = self.fc_2(feature_vector)
